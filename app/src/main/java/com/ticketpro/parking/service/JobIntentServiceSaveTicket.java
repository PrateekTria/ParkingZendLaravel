...
public class JobIntentServiceSaveTicket extends JobIntentService {

    private static final String TAG = "JobIntentServiceSaveTicket";
    private static final Logger log = Logger.getLogger("JobIntentServiceSaveTicket");
    public TPApplication TPApp;
    boolean SocketTimeOutFlag = false;
    public JobIntentServiceSaveTicket() {
        this.TPApp = TPApplication.getInstance();
    }

    public static void enqueueWork(Activity context, Intent serviceIntent) {
        enqueueWork(context, JobIntentServiceSaveTicket.class, 123, serviceIntent);
    }
    ...
    protected synchronized void onHandleWork(@NonNull Intent intent) {
        ...
        try {
            ...
            syncDevices(deviceInfo, "JobIntentServiceSaveTicket");

            /**
             * current ticket data is pending
             */
            ArrayList<Ticket> ticketsArray = Ticket.getPendingCurrentTickets(TPApplication.getSingleton().getMultipleCitation());
            ...
            /**
             * {comment: we are skipping the process of sending previous pending images while printing the ticket}
             */
           /* ArrayList<Ticket> ticketsArrayPI = Ticket.getPendingCurrentTickets(TPApplication.getSingleton().getMultipleCitation());
